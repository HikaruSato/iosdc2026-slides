...
全87枚（本編82枚・末尾の補足5枚）で、40分想定です。通常は本編を話します。
Optional detailは時間超過時だけの予備であり、省略を前提にした時間配分ではありません。
ライブデモ成功時は代替説明5枚（16〜20ページ）だけを省略し、77枚とデモで進めます。失敗時はライブ操作を切り上げ、代替5枚で説明します。
...
発表終了後の補足（83〜87ページ）にまとめています。

## 設定値の理由

[MomentNowの開発記事](https://zenn.dev/hs7/articles/080eac650f65ba)と現行コードを照合し、設定の理由を説明する5枚（29・35・37・39・54ページ）を追加しています。
記事が明示する採用理由、Apple等の一次資料が説明する性質・推奨、今回の具体的な設定例を分けて話します。出典と詳しい注意点は各ページのノートにあります。

- 配信H.264と保存HEVC、720pと1080pの使い分け。
- Y′CbCr 4:2:0・8-bit・BiPlanar・FullRangeの意味と、色変換・メモリー帯域の関係。
- H.264 High Autoとframe reorderingの役割。AutoをABRと混同しない。
- AAC・mono・44.1 kHz・音声bitrate。monoは現行アプリの選択で、Appleの配信仕様が要求するstereoとは区別。
...
80ページの振り返りでもこの点を率直に話し、実装時の動作確認と、利用実績・ユーザー評価に基づく最適化を区別します。
少人数・短時間・単一品質は想定した用途であり、利用者への適合性を本番の実績で十分に検証したという結論にはしません。
最後の「ありがとうございました」（82ページ）には、サンプルコードと開発記事のURLを掲載しています。

## 図解の見せ方

HLSを知らないiOSエンジニア向けに、図で役割・処理順・並行処理を確認してから要点コードを示します。
図解の改修では、72枚時点の10・26・27・34・37・39・51・57・58・63ページをそれぞれ2枚に分けています。その後、設定値の理由を説明する5枚を追加しました。
...
- 停止処理は77〜78ページ。Recorderの現在の終了順と、HTTP送信・写真保存の並行処理を分けます。
